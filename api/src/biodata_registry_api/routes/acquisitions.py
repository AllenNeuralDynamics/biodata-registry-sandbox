...
from biodata_registry_api.models.crud.core import AcquisitionCreate, AcquisitionUpdate, AcquisitionsPage, AcquisitionsFilter
from biodata_registry_api.models.core import Acquisitions, Subjects, Specimens
...
@router.get(
    "/acquisitions",
    tags=["core"],
    response_model=AcquisitionsPage,
...
async def get_acquisitions(
        ...
        filter_query: AcquisitionsFilter = FilterDepends(AcquisitionsFilter),
        session: AsyncSession = Depends(get_session),
):
    previous_id = decode_next_token(next_token)
    statement = select(Acquisitions).order_by(Acquisitions.id.asc())
    statement = filter_query.filter(statement)
    if previous_id is not None:
        statement = statement.where(Acquisitions.id > previous_id)
