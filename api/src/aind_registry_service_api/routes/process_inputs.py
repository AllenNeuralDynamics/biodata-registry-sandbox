...
@router.get("/process_inputs", tags=["process_inputs"])
async def get_process_inputs(
    ...
    base_statement = "SELECT * FROM process_inputs"
    or_statement = ""
    param_dict = dict()
...
@router.post("/process_inputs", tags=["process_inputs"])
async def create_process_inputs(
    ...
    base_statement = "INSERT INTO process_inputs (data_asset_id, process_id) VALUES (:data_asset_id, :process_id) RETURNING *;"
...
@router.delete("/process_inputs", tags=["process_inputs"])
async def delete_process_inputs(
    data_asset_id: int = Query(
        alias="data_asset_id",
    ),
    process_id: int = Query(
    ...
    base_statement = "DELETE FROM process_inputs WHERE data_asset_id = :space_admin_id AND process_id = :process_id RETURNING *;"
    param_dict = {"data_asset_id": data_asset_id, "process_id": process_id}
    statement = text(base_statement)
    result = await session.execute(statement, param_dict)
    rows = []
