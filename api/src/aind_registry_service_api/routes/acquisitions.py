...
@router.get("/acquisitions", tags=["acquisitions"])
async def get_acquisitions(
    ...
    base_statement = "SELECT * FROM acquisitions"
    or_statement = ""
    param_dict = dict()
    if acquisition_id is not None:
        or_statement = " WHERE id = :acquisition_id"
...
async def delete_acquisitions(
    acquisition_id: int = Query(
        alias="acquisition_id",
    ),
    session: AsyncSession = Depends(get_session),
):
    base_statement = "DELETE FROM acquisitions WHERE id = :acquisition_id RETURNING *;"
    param_dict = {"acquisition_id": acquisition_id}
    statement = text(base_statement)
    result = await session.execute(statement, param_dict)
    rows = []
...
@router.put("/acquisitions", tags=["acquisitions"])
async def update_acquisitions(
    acquisition_id: int = Query(
        None,
        alias="acquisition_id",
    ),
    ...
    base_statement = "UPDATE acquisitions"
    set_statement = ""
    param_dict = dict()
    ...
    if data is not None and not param_dict:
        ...
        param_dict["data"] = json.dumps(data)
    param_dict["acquisition_id"] = acquisition_id
    new_statement = f"{base_statement} {set_statement} WHERE id = :acquisition_id RETURNING *;"
    statement = text(new_statement)
    result = await session.execute(statement, param_dict)
    rows = []
    for row in result:
